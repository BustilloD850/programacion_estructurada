...
def entrada_producto(codigo=None, cantidad=None):
    try:
        if codigo is None:
        ...
        # Buscar el ID del producto usando su código
        cur.execute("SELECT id_producto FROM producto WHERE codigo = %s", (codigo,))
        fila = cur.fetchone()
        if fila is None:
            print(f"❌ No existe un producto con código '{codigo}'.")
            cur.close()
            conexion.close()
            return False

        id_producto = fila[0]  # El ID real del producto

        # Actualizar stock
        cur.execute("UPDATE producto SET stock = stock + %s WHERE id_producto = %s",
                    (cantidad, id_producto))

        # Registrar en recarga sin usuario ni comentario
        cur.execute("INSERT INTO recarga (id_producto, cantidad) VALUES (%s, %s)",
        ...
        print("✅ Entrada de producto registrada correctamente.")
        return True

    except Exception as e:
        print("❌ Error en entrada de producto:", e)
        return False

def salida_producto(id_usuario, codigo=None, cantidad=None):
    try:
        if codigo is None:
            codigo = input("Ingrese el CÓDIGO del producto: ").strip()
        if cantidad is None:
